...
@Slf4j
@Component
public class HanaCardStatementParser implements CardStatementParser {

    private static final List<String> DATE_KEYWORDS = List.of("이용일", "거래일자", "거래일", "승인일");
    private static final List<String> MERCHANT_KEYWORDS = List.of("이용가맹점", "가맹점", "이용처", "가맹점명");
    private static final List<String> AMOUNT_KEYWORDS = List.of("이용금액", "거래금액", "금액", "승인금액", "이용금액(원)");
    ...
    public List<ParsedTransaction> parse(InputStream xlsx) throws IOException {
        try (XSSFWorkbook workbook = new XSSFWorkbook(xlsx)) {
            ...
            if (header == null) {
                throw new IllegalArgumentException("하나카드 명세서 형식을 인식할 수 없습니다. 헤더 행에 이용일/이용가맹점/이용금액 컬럼이 있어야 합니다.");
            }
            ...
            for (int r = header.rowIndex + 1; r <= lastRow; r++) {
                ...
                try {
                    ...
                    log.warn("하나카드 명세서 {}행 파싱 실패: {}", r + 1, e.getMessage());
                }
            }
            return result;
        }
    ...
    private String buildNaturalKey(LocalDate date, String merchant, BigDecimal amount, Integer installmentMonths) {
        ...
        return String.format("HANA:%s:%s:%s:%s", date, merchant.trim(), amount.toPlainString(), monthsPart);
    }

    int[] parseInstallment(String raw) {
