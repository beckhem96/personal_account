...
> 갱신: 초기엔 "오늘 접수중"만 표시했으나, 대부분의 날에 화면이 비는 문제로 **접수 예정(시작 전) + 진행중**(= 마감일이 지나지 않은 단계)을 모두 표시하도록 넓혔다. 카드에 `시작 D-N`(예정) / `마감 D-N`(진행중) 배지로 구분. 판정 기준: `isOpenOrUpcoming(today, begin, end)` = 마감일이 오늘 이후. LH 탭도 동일하게 접수마감 제외 방식.
