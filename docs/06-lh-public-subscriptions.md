...
| 항목 | 결정 |
|------|------|
| 공급유형 범위 | 분양 전체 + 신혼희망타운 + **임대 전체 유형**(공공임대·국민임대·영구임대·행복주택·통합공공임대·매입임대·전세임대 등). ※ 초기엔 주요 임대 3종만 노출했으나 사용자 요청으로 06 임대 전체로 확대 |
...
findActiveToday():
  ...
  4. PAN_ID 기준 중복 제거
  5. 분양(SALE)/임대(RENT) 그룹으로 분리 반환 — 임대는 06 전체 유형 노출(유형 구분은 AIS_TP_CD_NM 배지로)
```
...
2. 임대유형 전체 노출 — 행복주택/국민임대/공공임대/매입임대/전세임대 모두 포함
