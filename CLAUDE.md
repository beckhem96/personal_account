...
# 또는 수동 실행
docker-compose up -d                          # 1. DB 실행
cd backend && ./gradlew bootRun               # 2. 백엔드 (포트 8080)
cd frontend && npm install && npm run dev     # 3. 프론트 (포트 5173)
```

**AI 에이전트용 행동 가이드는 `AGENTS.md` 참조.**
...
### 연말정산 (소득공제)
- 최저 사용금액: 총급여의 25% (이 금액을 초과한 사용분만 공제 대상)
- 신용카드 공제율: 15%
- 체크카드/현금영수증 공제율: 30%
- 전통시장 공제율: 40% (추가공제 한도 100만원)
- 대중교통 공제율: 40% (추가공제 한도 100만원)
- 일반 공제한도: 총급여 7천만원 이하 → 300만원, 초과 → 250만원
- 최저사용금액 차감 시 공제율 낮은 항목(신용카드)부터 차감하여 사용자에게 유리하게 계산
